...
public class CompleteGraph implements Graph {
	 ...
	 * @param cost          : 2D array representing a complete graph, where each
	 *                      vertex is a pickup or delivery point, or the depot
	 * @param costTmp       : 2D array allowing us to make a link between map and
	 *                      cost
	 * @param precedence    : Map storing a the precedence array of each vertex of
	 *                      cost after Djikstra algorithm
	 * @param nameNodeCost  : Map allowing us to make a link between an Intersection
	 *                      and the integer used to represent it
	 * 
	 *                      First, the constructor creates an array representing the
	 *                      city map. It is a rather big array, and not complete at
	 *                      all Then, it creates the CostGraph array, which
	 *                      represents a complete graph, where each vertex is a
	 *                      delivery or pickup point, or the depot
	 */
	...
	/**
	 * Constructor. First, the constructor creates an array representing the city
	 * map. It is a rather big array, and not complete at all Then, it creates the
	 * CostGraph array, wich represents a complete graph, where each vertex is a
	 * delivery or pickup point, or the depot
	 * 
	 * @param cm  The city map
	 * @param sor The set of requests to build the complete graph from
	 */
	 ...
	 * Constructor. First, the constructor creates an array representing the city
	 * map. It is a rather big array, and not complete at all Then, it creates the
	 * CostGraph array, which represents a complete graph, where each vertex is a
	 * delivery or pickup point, or the depot
	 * 
	 * @param cm     The city map
	 * @param points list of delivery or pickup points, and the depot
	 */
	...
	/**
	 * Convert the map into a 2D array of intersections containing distances between
	 * the origin and the destination of the segment
	 * 
	...
	/**
	 * Create a complete graph of a shortest paths between each vertex, where each
	 * vertex is a delivery or pickup point, or the depot.
	 * 
	...
	/**
	 * Apply the Dijkstra algorithm from firstNode. This method also compute the
	...
	/*
	 * private boolean isDijkstraFinished(int indexBegin, int indexEnd) { return
	 * (indexBegin == indexEnd); }
	 * 
	 * private int findIndexOfMinCostOfVisitedNodes(int[] colorNodes, float[] d) {
	 * float min = INFINITE; int index = 0; for (int i = 0; i< colorNodes.length;
	 * i++) { int currentNode = i; if (d[currentNode] < min && (colorNodes[i] > 0))
	 * { min = d[currentNode]; index = currentNode; } } return index; }
	 */

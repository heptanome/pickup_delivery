...
/**
 * Graphical representation of an Intersection Color code: white: random
 * intersection blue : pickup point magenta : delivery point yellow : departure
 */
public class GraphicalPoint {
	 ...
	 * Contructor
	 * 
	 * @param i        the intersection that will be represented by the
	 *                 GraphicalPoint
	 ...
	 * @return true if the (x,y) belongs to the circle of center(xPixel,yPixel) and
	 *         of radius size/2
	 */
	public boolean isClicked(int x, int y) {
		if (Math.sqrt((x - (xPixel + size / 2)) * (x - (xPixel + size / 2))
				+ (y - (yPixel + size / 2)) * (y - (yPixel + size / 2))) <= size / 2) {
			return true;
		} else {
			return false;
		}
